...
- **`App.tsx`** — Routes `GamePhase → Screen`. `TableFelt` always rendered behind screens (blurred when `phase !== 'BETTING'`). BETTING → `BettingTableScreen`. QUESTION branches by mode: `closest` → `ClosestScreen`, `kerri && isGladiator` → `GladiatorSelfScreen`, else → `QuestionScreen`.
- **`components/screens/`** — Screen components. `LobbyScreen` shows START button only to host (`myId === gameState.hostId`). `AnnounceScreen` has sequential staggered animation: label (t=0) → mode name + description (t=0.2s) → topic (t=1.0s). `BettingTableScreen` handles all modes; in kerri mode crowd sees WIN/LOSE selector + bank bet x4 UI (option buttons + chip amount picker, emits `place_bank_bet` on confirm).
