...
- **`game/GameRoom.ts`** — Core state machine. Extends EventEmitter. `hostId` = socket id of first `addPlayer` call; `start(requesterId)` rejects if requester is not host. Phase transitions via `schedulePhase()` / `onPhaseEnd()` — **all modes now go through BETTING** (not just kerri/all). In kerri mode, `selectGladiator()` runs at ANNOUNCE→BETTING transition. `placeBankBet(playerId, optionIndex, amount)` stores crowd's side-bet on which MC option the kerri will choose. `calculateResults()` calls `applyBankBets()` after main pool distribution. Emits three events wired by GameEngine: `broadcast`, `broadcastExcept`, `sendToPlayer`. `getStateForPlayer(playerId)` returns personalized state — crowd players receive `gladiatorAnswer` (= correct answer string), kerri player does not. `getPublicState()` strips `bankBet` from player objects.
...
- **`socket/handlers.ts`** — Seven handlers: `join_game`, `start_game` (passes `socket.id` as requester), `place_bet`, `place_bank_bet`, `submit_answer`, `gladiator_hover`, `disconnect`.

### Client (`client/src/`)

- **`socket.ts`** — Socket.IO singleton, `autoConnect: false`. URL from `VITE_SERVER_URL` env var, falls back to `http://localhost:3001`.
- **`store/gameStore.ts`** — Zustand store. All socket listeners live here. `roundResults` cleared on ANNOUNCE. `bankBets: Record<playerId, {optionIndex, amount}>` updated via `bank_bet_updated` events, cleared on ANNOUNCE. Selectors: `selectMe`, `selectIsGladiator`. Betting chip state is component-local in `BettingTableScreen`.
- **`App.tsx`** — Routes `GamePhase → Screen`. `TableFelt` always rendered behind screens (blurred when `phase !== 'BETTING'`). Full routing: `LOBBY→LobbyScreen`, `ANNOUNCE→AnnounceScreen`, `BETTING→BettingTableScreen`, `QUESTION_TEXT→QuestionTextScreen`, `QUESTION` branches by mode (`closest→ClosestScreen`, `top5→Top5Screen`, `kerri&&isGladiator→GladiatorSelfScreen`, else `QuestionScreen`), `REVEAL→RevealScreen`, `LEADERBOARD→LeaderboardScreen`, `GAME_OVER→GameOverScreen`.
- **`components/screens/`** — Screen components. `LobbyScreen` shows START button only to host (`myId === gameState.hostId`). `AnnounceScreen` has sequential staggered animation: label (t=0) → mode name + description (t=0.2s) → topic (t=1.0s). `BettingTableScreen` handles all modes; in kerri mode crowd sees WIN/LOSE selector + bank bet x4 UI (option buttons + chip amount picker, emits `place_bank_bet` on confirm). `RevealScreen` shows win delta as animated physical chips (`WinChips` component, 60ms stagger).
...
The betting phase uses a physical chip system on a round poker table (desktop only, fixed **1040×660** scene).
...
- **`components/screens/BettingTableScreen.tsx`** — `LayoutGroup` wraps all chip elements. Local state: `myStack`, `placedIds`, `pendingTarget`, `bankBetTarget`, `bankBetAmount`. `placeChip(denom)` finds first unplaced chip of that denom. Bank bet UI: shows 4 MC options; correct one (matching `gladiatorAnswer`) has green border + non-clickable; wrong options clickable → select `bankBetTarget` (index); `+10/+20/+50/+100` buttons for `bankBetAmount`. Confirm emits `place_bet` and optionally `place_bank_bet`.
...
All modes go through BETTING. In kerri mode, `selectGladiator()` runs at ANNOUNCE→BETTING transition.
